...
- **Shared prompts** — `app/prompt_builder.py` centralizes domain-knowledge and language-rule constants used by all three worker subgraphs.
...
- **Redis streaming** — `POST /chat/stream` appends events to the Redis Stream `agent:stream:{thread_id}:{run_id}` (replayable via `XREAD`) and returns `{thread_id, channel}`; falls back to inline SSE when Redis is unavailable. Event names are `progress`, `token`, `heartbeat`, `paused`, `done`, `error`; every run ends with exactly one terminal event. `/approve/stream` streams the resumed HITL run over the same contract.
...
- **Domain registry** — `app/domain_registry.py` provides deterministic explanations for domain terms (hot/cold, group sizes, `probability`, `lucky_numbers`, `saved_numbers`, `luck`). The supervisor's `direct_domain` route serves these without an LLM call for `domain_explanation` and question-form `ambiguous` messages.
...
- `REDIS_URL` — Optional Redis URL; enables the Redis Streams relay for `/chat/stream` (falls back to inline SSE if unset)
- `ALLOWED_ISSUERS` — Optional comma-separated list of trusted JWT `iss` values (empty = issuer not checked; signature + audience always verified)
- `AGENT_CODE_ROOT` — Root for `read_code`/`list_files`/`edit_file` (default `/app`). Edits only persist when the code root is a mounted volume — the dev compose mounts `./app:/app/app`; the prod image has no mount, so `edit_file` changes are ephemeral there.
